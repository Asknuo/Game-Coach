...
async def latest_tips():
    state = await redis_store.get_state("default")
    return {
        "state": state,
        "memory": {
...
async def _record_advice_context(
    tip: dict,
    result: dict,
    ...
    await redis_store.record_advice_given(
        "default",
        skill=tip["skill"],
...
async def _check_advice_feedback(payload: dict) -> None:
    """每帧 state 到达时检查建议反馈.

    状态机：followed → 加置信度；pending → 下帧继续观察；expired → 降置信度。
    """
    status, skill, reason = await redis_store.check_advice_followed("default", payload)
    if status == "followed":
        metrics["advice_followed"] += 1
        new_conf = await redis_store.adjust_skill_confidence("default", skill, True)
        logger.info("Feedback: [%s] advice followed (%s) → conf %.2f", skill, reason, new_conf)
    elif status == "expired":
        metrics["advice_expired"] += 1
        new_conf = await redis_store.adjust_skill_confidence("default", skill, False)
...
def _handle_lcu_event(event: CoachEvent) -> bool:
...
async def _review_on_disconnect(state: GameState) -> None:
    ...
    initial = _build_coach_state(event, state, [], 3)
    try:
        result = await asyncio.wait_for(coaching_graph.ainvoke(initial), timeout=30.0)
...
async def _summarize_on_disconnect(state: GameState | None) -> None:
    ...
    try:
        await _review_on_disconnect(state)
    except Exception:
        logger.exception("Review on disconnect failed")

    # 2. 结构化摘要沉淀到 history + facts（带真实 KDA）
    summary = {
        "champion": ap.champion_name or memory.user.current_champion,
    ...
    try:
        await asyncio.wait_for(engine.summarize_game("default", summary), timeout=30.0)
    except asyncio.TimeoutError:
...
class CollectorSession:
    ...
    async def handle_coaching(self, item: dict) -> None:
        ...
        metrics["tips_published"] += 1
        engine.update_top_of_mind(event, snapshot)
        ...
        await _broadcast_tip_json(tip_json)
        await _record_advice_context(tip, result, self.latest_state)
    ...
    async def _on_state(self, msg: WSMessage) -> None:
        ...
        await redis_store.save_state("default", msg.payload)
        await _check_advice_feedback(msg.payload)
